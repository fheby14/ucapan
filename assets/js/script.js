...
// Dirubah 
const pengirim = "baba";
const nomorWa = "6285212634626" // awalan nomor 0 harus di awalin 62
const pesan = `bunda jangan marah lama lama yah bunda baba takut bunda :(, bunda harus kembali berseyum lagi bunda :), baba janji tidak seperti ini lagi bunda, maafin baba nya yah bunda.`;
...
hilang1.addEventListener('click', () => {
  ...
  Swal.fire({
    ...
    imageHeight: 120,
    title: 'bunda baba minta maaf :( bunda',
    html: `<input type="text" id="login" class="swal2-input" placeholder="bunda">`,
    confirmButtonText: 'Kirim',
    focusConfirm: false,
    preConfirm: () => {
      const login = Swal.getPopup().querySelector('#login').value
      if (!login) {
        Swal.showValidationMessage(`bunda`)
      }
      return { login }
    }
  }).then((result) => {
    ...
    let txt = `Bunda, baba mau minta maaf kepada ${result.value.login}, karena hari ini baba membuat bunda marah, kesal, dan badmood. ${pesan}`;
